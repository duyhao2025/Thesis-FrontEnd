...
export default function StaffTopicProposalsPage() {
  ...
  const load = () => {
    setLoading(true);
    api.get("/staff/topic-proposals/lecturer-approved")
  ...
  const handleApprove = async (id: string) => {
    if (!confirm("Duyệt đề xuất này? Một Topic mới sẽ được tạo và đưa vào hàng chờ phân công giảng viên.")) return;
    setActionId(id);
    try {
      await api.post(`/staff/topic-proposals/${id}/approve`, {});
  ...
  const handleReject = async () => {
    ...
    try {
      await api.post(`/staff/topic-proposals/${selectedProposal.id}/reject`, { reason: rejectReason });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Duyệt đề xuất (sau GV)</h1>
          <p className="text-sm text-gray-500">Danh sách đề xuất mà Giảng viên đã duyệt - thuộc Khoa/Chuyên ngành của bạn</p>
        </div>
      </div>

      <DataTable columns={columns} data={proposals} loading={loading} rowKey="id" emptyMessage="Không có đề xuất chờ duyệt" />
